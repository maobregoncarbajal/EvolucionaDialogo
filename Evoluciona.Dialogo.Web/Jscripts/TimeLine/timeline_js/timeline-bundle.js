...
Timeline.GregorianDateLabeller.getQuarterSymbol = function(A) {
    return Timeline.GregorianDateLabeller.quarterSymbol[A];
};
Timeline.GregorianDateLabeller.prototype.defaultLabelInterval = function(B, C) {
    ...
    switch (C) {
        ...
        case SimileAjax.DateTime.WEEK:
            D = Timeline.GregorianDateLabeller.getMonthName(B.getUTCMonth(), this._locale) + " " + B.getUTCDate();
            break;
        case SimileAjax.DateTime.CAMPANHA:
            var date1 = "01-01-" + B.getUTCFullYear();
            var date2 = B.getUTCDate() + "-" + (B.getUTCMonth() + 1) + "-" + B.getUTCFullYear();
            var numDia = daysBetween(date1, date2);

            var numCampanha = parseInt(numDia / 20.3) + 2;
        ...
        case SimileAjax.DateTime.MONTH:
            ...
            var A = B.getUTCMonth();
            if (A != 0) {
                if (C == Timeline.DateTime.QUARTER) {
                    D = Timeline.GregorianDateLabeller.getQuarterSymbol(this._locale) + ((A / 3) + 1);
                } else {
        ...
        case SimileAjax.DateTime.MILLENNIUM:
            ...
            if (E > 0) {
                ...
                D = (1 - E) + "BC";
            } F = (C == SimileAjax.DateTime.MONTH) || (C == SimileAjax.DateTime.QUARTER) || (C == SimileAjax.DateTime.DECADE && E % 100 == 0) || (C == SimileAjax.DateTime.CENTURY && E % 1000 == 0);
            break;
