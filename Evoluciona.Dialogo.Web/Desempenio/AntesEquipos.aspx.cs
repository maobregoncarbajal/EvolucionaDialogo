...
namespace Evoluciona.Dialogo.Web.Desempenio
{
    ...
    public partial class AntesEquipos : Page
    {
        #region Variables
        ...
        protected void btnGuardarDes_Click(object sender, EventArgs e)
        {
            ...
            var lstCargarCriticasProcesadas = _daProceso.ListaCargarCriticasProcesadas(CodigoUsuarioProcesado,
                ...
                case Constantes.RolGerenteZona:
                {
                    ...
                    if (paisesLetsExitosas.IndexOf(ObjResumenBe.prefijoIsoPais, StringComparison.Ordinal) != -1)
                        ...
                        else
                        {
                            ...
                            var lstLets = oblLet.ObtenerLetsPorZona(ObjResumenBe.prefijoIsoPais, ObjResumenBe.codigoUsuario,
                            ...
                            {
                                if (lstCodsLets.Contains(lstLet.CodigoConsultoraLet)) continue;
                            ...
                            foreach (var codLet in lstDatosLets)
                            {
                                ...
                                foreach (var cmpnh in strCampanias)
                                {
                                    ...
                                    if (let != null)
                                    {
                                        var urlImagen = RetornarImagen(@let.EstadoCamp);
        ...
        {
            var procesados = (List<BeCriticas>)Session["criticasProcesadas"];
            if (procesados == null || procesados.Count <= 0) return;
        ...
        private static List<BeCriticas> ObtenerCriticasRestantes(IEnumerable<BeCriticas> criticasTotales, List<BeCriticas> criticasProcesadas)
        {
            return (from item in criticasTotales let objetoEncontrado = criticasProcesadas.Find(objCriticaBusca => objCriticaBusca.documentoIdentidad.Trim() == item.documentoIdentidad.Trim()) where objetoEncontrado == null select item).ToList();
        }

        private BeCriticas BuscarCriticasDisponibles(string valorBuscar)
        {
            var criticas = (List<BeCriticas>)Session["criticasDisponibles"];
            return criticas != null ? criticas.FirstOrDefault(item => item.documentoIdentidad.Trim() == valorBuscar.Trim()) : null;
        }
        ...
        private void ValidarPeriodoEvaluacion()
        {
            ...
            if (dtPeriodoEvaluacion == null) return;
        ...
        private void Crear_Actualizar(string valor, string nombrePersona, string textoIngresado)
        {
            ...
            {
                if (item.NavigateUrl != valor) continue;
        ...
        private static string RetornarImagen(string estado)
        {
            string nombreImagen;
