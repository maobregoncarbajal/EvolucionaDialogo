...
namespace Evoluciona.Dialogo.Web.Adapters
{
    public class MenuAdapter : System.Web.UI.WebControls.Adapters.MenuAdapter
    {
        ...
        private void RegisterScripts()
        {
            ...
            // TreeViewAdapter.js
            string resource = "Evoluciona.Dialogo.Web.Scripts.MenuAdapter.js";
            ...
            resource = "Evoluciona.Dialogo.Web.Styles.Menu.css";
            filePath = Page.ClientScript.GetWebResourceUrl(type, resource);

            // if filePath is not empty, embedded CSS exists -- register it
            if (!String.IsNullOrEmpty(filePath))
            {
                string cssTag = "<link href=\"" + /*Page.ResolveUrl(filePath)*/ "Styles/Menu.css"  + "\" type=\"text/css\" rel=\"stylesheet\"></link>";
            ...
            if ((menu != null) && (item != null) && (writer != null))
            {
                ...
                else
                {
                    if (IsLink(item))
                    {
                        ...
                        {
                            writer.WriteAttribute("href", Page.ClientScript.GetPostBackClientHyperlink(menu, "b" + /*item.ValuePath.Replace(menu.PathSeparator.ToString(), "\\")*/ item.ValuePath , true));
                        }
